...
FC-GNN training takes $11$--$16$\,s per dataset
...
off-line once before calibration. \rev{However, deploying FC-GNN in large, dynamic enterprise networks may introduce scalability bottlenecks during continuous recalibration. Frequent graph updates require re-computing community assignments and running the fuzzy rule evaluation over new topological structures, which may be costly for high-throughput streaming environments. Future work could explore incremental community detection and lightweight localized calibration updates to maintain efficiency.}
...
expose this failure mode. \rev{It is critical to distinguish between theoretical guarantees—which hold strictly under exchangeability—and empirical coverage, which degrades proportionally to the severity of the drift. Under severe adversarial evasion or evolving attack communities, empirical coverage can fall significantly below the target $1-\alpha$.}
Future work should incorporate sliding-window recalibration
\citep{cp_cps26} or non-exchangeable CP \citep{ncpnet25} to address drift \rev{and recover empirical validity in dynamic settings}.
...
\item \rev{\textbf{Graph Construction Sensitivity.}
      Our evaluation does not extensively test the model's sensitivity to varying graph construction parameters, such as edge density thresholds, temporal time-window sizes, or alternative edge definitions. Future work should systematically analyze how these heuristic choices impact both prediction accuracy and conformal coverage.}
\end{enumerate}
