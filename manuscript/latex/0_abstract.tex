% sn-jnl defines \abstract and \keywords as one-argument COMMANDS
% (\def\abstract#1{...}), not environments. Written as
% \begin{abstract}...\end{abstract}, \abstract silently consumed the very
% next token as its argument -- the leading "G" of "Graph" -- and the rest
% was typeset as ordinary body text, with no Abstract heading and no
% keywords anywhere in the document.
\abstract{Graph Neural Networks (GNNs) have demonstrated strong performance for
...
N-BaIoT, and ToN-IoT — confirm that FC-GNN provides theoretical conditional
coverage under stated exchangeability assumptions (empirical mean coverage $0.903$,
reaching target on $3/7$ datasets at $\alpha\!=\!0.10$) while
outperforming the leading Mondrian CP baseline (RR-GNN) in prediction-set
efficiency on five datasets, and
...
\keywords{Graph neural networks, Conformal prediction, Fuzzy logic, Cybersecurity, Anomaly detection, Uncertainty quantification, Interpretability}
